...
def predict_future_price(commodity, state, year, month):
    """
    Predict food price for a specific commodity, state, year, and month.
    ...
    year : int
        Target year (e.g., 2026, 2027)
    month : int
        Target month (1-12)
    
    Returns:
    --------
    dict : Contains predicted price and metadata
    """
    ...
    if commodity not in trained_models:
        return {
            'status': 'ERROR',
            'message': f'Model for {commodity} not found. Available: {list(trained_models.keys())}'
        }
    
    # Find the most recent data for this commodity-state combination
    commodity_state_data = df[(df['commodity'] == commodity) & (df['state'] == state)].sort_values('date').tail(12)
    ...
    # Get the most recent lag features
    recent_price = commodity_state_data['price'].iloc[-1]
    price_lag_1 = commodity_state_data['price'].iloc[-1]
    price_lag_3 = commodity_state_data['price'].iloc[-3] if len(commodity_state_data) >= 3 else recent_price
    price_lag_6 = commodity_state_data['price'].iloc[-6] if len(commodity_state_data) >= 6 else recent_price
    price_lag_12 = commodity_state_data['price'].iloc[-12] if len(commodity_state_data) >= 12 else recent_price
    
    rolling_3 = commodity_state_data['price'].iloc[-3:].mean()
    rolling_6 = commodity_state_data['price'].iloc[-6:].mean()
    rolling_12 = commodity_state_data['price'].iloc[-12:].mean()
    
    # Get state and commodity average prices
    state_avg_price_lag1 = df[(df['state'] == state) & (df['commodity'] == commodity)]['price'].mean()
    ...
    price_momentum = commodity_state_data['price'].iloc[-1] - commodity_state_data['price'].iloc[-6] if len(commodity_state_data) >= 6 else 0
    ...
    if metadata['type'] == 'volatile':
        variance_scale = metadata['variance_scale']
        mean_diff = metadata['mean_diff']
        prediction = (prediction - np.mean(feature_values)) * variance_scale + np.mean(feature_values) + mean_diff
    ...
    return {
        ...
        'confidence': 'High' if metadata['type'] == 'normal' else 'Medium (Volatile commodity)'
    }
...
# Example predictions
examples = [
    ('Rice (local)', 'Adamawa', 2026, 3),
    ('Yam', 'Borno', 2026, 6),
    ('Beans (red)', 'Yobe', 2026, 9),
    ('Oranges', 'Adamawa', 2027, 1),
    ('Tomatoes', 'Borno', 2027, 12)
]
